...
// if (["add", "edit", "remove"].includes(operation)) {
//     if (operation === "add") {
//         var newItem = { name: item[0], category: item[1], quantity: item[2], price: item[3], unit: item[4], added: new Date(), customField: item[5] || {} };
//         items.push(newItem);
...
//         transactions.push({ type: "add", item: newItem });
//     } else if (operation === "edit" && items[operationParameter]) {
//         items[operationParameter] = { ...items[operationParameter], name: item[0], category: item[1], quantity: item[2], price: item[3], unit: item[4], customField: item[5] || {} };
//         transactions.push({ type: "edit", old: items[operationParameter], new: item.slice(1) });
//     } else if (operation === "remove" && items[operationParameter]) {
//         transactions.push({ type: "delete", item: items[operationParameter] });
//         items.splice(operationParameter, 1);
//     }
//     console.log("=== Dashboard ===\nItems: " + items.length + "\nTotal: $" + items.reduce((total, x) => total + x.quantity * x.price, 0).toFixed(2) + "\nCats: " + categories.join(', '));
// }
// if (["sale", "restock"].includes(operation)) {
//     for (let item of items) {
//         if (item.name === operationParameter) {
//             if (operation === "sale" && item.quantity >= item[1]) {
//                 item.quantity -= item[1];
//                 transactions.push({ type: "sale", item: item, quantitySold: item[1], date: new Date() });
//                 console.log(`Sold ${item[1]} ${item.unt} of ${item.n}`);
//             } else if (operation === "restock") {
//                 item.quantity += item[1];
//                 transactions.push({ type: "restock", item: item, qunatityRestocked: item[1], date: new Date() });
//                 console.log(`Restocked ${item[1]} ${item.unt} of ${item.n}`);
//             }
//             break;
//         }
//     }
...
//if (a === "srch") console.log(items.filter(x => [x.n, x.cat, x.prc].some(v => v.toString().toLowerCase().includes(b[0].toLowerCase()))));
//if (a === "vwI") console.log("=== Inv ===", items);
//if (a === "xprtAll") console.log("CSV:\n" + ["Name,Category,Quantity,Price,Unit,AddedAt"].concat(items.map(x => Object.values(x).join(','))).join('\n'));
//if (a === "vwAllT") console.log("Transactions:\n", t);
//if (a === "vwIAg") console.log(items.map(x => `${x.n}: ${Math.floor((new Date() - new Date(x.added)) / (1000 * 60 * 60 * 24))}d`).join('\n'));
//if (a === "Imprt") b[0].forEach(x => doStuff("add", [x.n, x.cat, x.quantity, x.price, x.unit]));
//if (a === "addFld" && !f[b[0]]) f[b[0]] = null;
//if (a === "udCFld") items.find(x => x.n === b[0])?.customField[b[1]] = b[2];